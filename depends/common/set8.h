/*
 * This is the implementation of VS2008 set class which is
 * compatible with VS2015 set structure, achieved by removing
 * allocators and comparator members
 */
...
_STD_BEGIN
		...
		// TEMPLATE CLASS _Tree_set8_nod
template<class _Traits>
	class _Tree_set8_nod
		: public _Traits	// traits form ultimate base
	{	// base class for _Tree_set8_ptr to hold allocator _Alnod()
protected:
	struct _Node;
	friend struct _Node;
	...
	_Tree_set8_nod(const key_compare& _Parg,
		allocator_type _Al)
    ...
    typedef typename allocator_type::template rebind<_Node>::other _Alnod_t;
    static _Alnod_t _Alnod()
        {
            return _Alnod_t();
        }
	};

		// TEMPLATE CLASS _Tree_set8_ptr
template<class _Traits>
	class _Tree_set8_ptr
		: public _Tree_set8_nod<_Traits>
	{	// base class for _Tree_set8_val to hold allocator _Alptr()
 //#if _HAS_ITERATOR_DEBUGGING
public:
 //#else /* _HAS_ITERATOR_DEBUGGING */
//protected:
 //#endif /* _HAS_ITERATOR_DEBUGGING */
	typedef _Tree_set8_nod<_Traits> _Mybase;
	typedef typename _Mybase::_Node _Node;
	typedef typename _Mybase::_Nodeptr _Nodeptr;
	typedef typename _Traits::allocator_type allocator_type;
	typedef typename _Traits::key_compare key_compare;

	_Tree_set8_ptr(const key_compare& _Parg,
		allocator_type _Al)
		: _Tree_set8_nod<_Traits>(_Parg, _Al)//, _Alptr()(_Al)
    ...
    typedef typename allocator_type::template rebind<_Nodeptr>::other _Alptr_t;
    static _Alptr_t _Alptr()
        {
            return _Alptr_t();
        }
	};

		// TEMPLATE CLASS _Tree_set8_val
template<class _Traits>
	class _Tree_set8_val
		: public _Tree_set8_ptr<_Traits>
	{	// base class for _Tree to hold allocator _Alval()
protected:
	typedef typename _Traits::allocator_type allocator_type;
	typedef typename _Traits::key_compare key_compare;

	_Tree_set8_val(const key_compare& _Parg,
		allocator_type _Al)
		: _Tree_set8_ptr<_Traits>(_Parg, _Al)//, _Alval()(_Al)
		{	// construct base, and allocator from _Al
		}

	//allocator_type _Alval();	// allocator object for values stored in nodes
    static allocator_type _Alval()
        {
        return allocator_type();
        }
	};

		// TEMPLATE CLASS _Tree_set8
template<class _Traits>
	class _Tree_set8
		: public _Tree_set8_val<_Traits>
	{	// ordered red-black tree for [multi_]{map set8}
public:
	typedef _Tree_set8<_Traits> _Myt;
	typedef _Tree_set8_val<_Traits> _Mybase;
...
public:
	...
	{	// iterator for nonmutable _Tree_set8
	public:
		friend class _Tree_set8<_Traits>;
 ...
 //#if _HAS_ITERATOR_DEBUGGING
 //#define _TREE_CONST_ITERATOR_SET8(ppnode)	const_iterator(ppnode, this)
 ...
 //#else
 #define _TREE_CONST_ITERATOR_SET8(ppnode)	const_iterator(ppnode)
 ...
 //#if _HAS_ITERATOR_DEBUGGING
	...
	//			{
	//			_DEBUG_ERROR("map/set8 iterator not dereferencable");
 ...
 //#if _HAS_ITERATOR_DEBUGGING
	...
	//			{
	//			_DEBUG_ERROR("map/set8 iterators incompatible");
 ...
 //#if _HAS_ITERATOR_DEBUGGING
	...
	//			|| _Ptr == 0)
	//			{
	//			_DEBUG_ERROR("map/set8 iterator not decrementable");
	//			_SCL_SECURE_INVALID_ARGUMENT;
	//			}
 //#else
	//		_SCL_SECURE_VALIDATE(this->_Has_container());
...
//#endif
 ...
 //#if _HAS_ITERATOR_DEBUGGING
	...
	//			|| _Isnil(_Ptr))
	//			{
	//			_DEBUG_ERROR("map/set8 iterator not incrementable");
	//			_SCL_SECURE_OUT_OF_RANGE;
	//			}
 //#else
	//		_SCL_SECURE_VALIDATE(this->_Has_container());
 ...
 //#endif /* _HAS_ITERATOR_DEBUGGING */
	...
	{	// iterator for mutable _Tree_set8
	public:
		typedef bidirectional_iterator_tag iterator_category;
		typedef _Dift difference_type;
 ...
 //#if _HAS_ITERATOR_DEBUGGING
 //#define _TREE_ITERATOR_SET8(ppnode)	iterator(ppnode, this)
 ...
 //#else /* _HAS_ITERATOR_DEBUGGING */
 #define _TREE_ITERATOR_SET8(ppnode)	iterator(ppnode)
 ...
 //#endif /* _HAS_ITERATOR_DEBUGGING */
	...
	explicit _Tree_set8(const key_compare& _Parg,
	...
	_Tree_set8(const value_type *_First, const value_type *_Last,
	...
	iterator begin()
		...
		return (_TREE_ITERATOR_SET8(_Lmost()));
		}

	const_iterator begin() const
		{	// return iterator for beginning of nonmutable sequence
		return (_TREE_CONST_ITERATOR_SET8(_Lmost()));
		}

	iterator end()
		{	// return iterator for end of mutable sequence
		return (_TREE_ITERATOR_SET8(_Myhead));
		}

	const_iterator end() const
		{	// return iterator for end of nonmutable sequence
		return (_TREE_CONST_ITERATOR_SET8(_Myhead));
		}

	iterator _Make_iter(const_iterator _Where) const
		{	// make iterator from const_iterator
		return (iterator(_TREE_ITERATOR_SET8(_Where._Ptr)));
		}
	...
	_Pairib insert(const value_type& _Val)
		...
		if (this->_Multi)
			...
			iterator _Where = _TREE_ITERATOR_SET8(_Wherenode);
			if (!_Addleft)
 ...
 //#if _HAS_ITERATOR_DEBUGGING
	...
	//		_DEBUG_ERROR("map/set8 insert iterator outside range");
 //#endif /* _HAS_ITERATOR_DEBUGGING */
 ...
 //#if _HAS_ITERATOR_DEBUGGING
	...
	//		_DEBUG_ERROR("map/set8 erase iterator outside range");
 ...
 //#else /* _HAS_ITERATOR_DEBUGGING */
		if (_Isnil(_Where._Mynode()))
			_THROW(out_of_range, "invalid map/set8<T> iterator");
		_Nodeptr _Erasednode = _Where._Mynode();	// node to erase
		++_Where;	// save successor iterator for return
 //#endif /* _HAS_ITERATOR_DEBUGGING */
 ...
 //#endif /* _HAS_ITERATOR_DEBUGGING */
	...
	iterator lower_bound(const key_type& _Keyval)
		...
		return (_TREE_ITERATOR_SET8(_Lbound(_Keyval)));
		}

	const_iterator lower_bound(const key_type& _Keyval) const
		{	// find leftmost node not less than _Keyval in nonmutable tree
		return (_TREE_CONST_ITERATOR_SET8(_Lbound(_Keyval)));
		}

	iterator upper_bound(const key_type& _Keyval)
		{	// find leftmost node greater than _Keyval in mutable tree
		return (_TREE_ITERATOR_SET8(_Ubound(_Keyval)));
		}

	const_iterator upper_bound(const key_type& _Keyval) const
		{	// find leftmost node greater than _Keyval in nonmutable tree
		return (_TREE_CONST_ITERATOR_SET8(_Ubound(_Keyval)));
		}
...
protected:
	...
	_Paircc _Eqrange(const key_type& _Keyval) const
		...
		const_iterator _First = _TREE_CONST_ITERATOR_SET8(_Lonode);
		const_iterator _Last = _TREE_CONST_ITERATOR_SET8(_Hinode);
	...
	_Pairii _Eqrange(const key_type& _Keyval)
		...
		iterator _First = _TREE_ITERATOR_SET8(_Lonode);
		iterator _Last = _TREE_ITERATOR_SET8(_Hinode);
	...
	iterator _Insert(bool _Addleft, _Nodeptr _Wherenode,
		...
		if (max_size() - 1 <= _Mysize)
			_THROW(length_error, "map/set8<T> too long");
		_Nodeptr _Newnode = _Buynode(_Myhead, _Wherenode, _Myhead,
			_Val, _Red);

		++_Mysize;
		if (_Wherenode == _Myhead)
			{	// first node in tree, just set8 head values
			_Root() = _Newnode;
		...
		return (_TREE_ITERATOR_SET8(_Newnode));
		}
 ...
 //#endif /* _HAS_ITERATOR_DEBUGGING */
	...
	static void _Xinvarg()
		...
		_THROW(invalid_argument, "invalid map/set8<T> argument");
		}
...
class _Container_set8_base_no_alloc
	{	// base of all containers
	public:
	void _Swap_aux(_Container_set8_base_no_alloc&)
		{
...
template<class _Alloc>
	class _Container_set8_base_aux_alloc_no_alloc
		: public _Container_set8_base_no_alloc
	{ // base class for containers to avoid holding allocator _Alaux
	protected:
	explicit _Container_set8_base_aux_alloc_no_alloc(_Alloc) { }

	_Container_set8_base_aux_alloc_no_alloc(const _Container_set8_base_aux_alloc_no_alloc&) { }

	_Container_set8_base_aux_alloc_no_alloc& operator=(const _Container_set8_base_aux_alloc_no_alloc&)
		{
		return *this;
		}

	~_Container_set8_base_aux_alloc_no_alloc() { }
	};
		// TEMPLATE CLASS _Tset8_traits
template<class _Kty,	// key/value type
	class _Pr,	// comparator predicate type
	class _Alloc,	// actual allocator type (should be value allocator)
	bool _Mfl>	// true if multiple equivalent keys are permitted
	class _Tset8_traits
		: public _Container_set8_base_aux_alloc_no_alloc<_Alloc>
	{	// traits required to make _Tree_set8 behave like a set8
public:
	...
	_Tset8_traits(_Pr _Parg, _Alloc _Al)
		: _Container_set8_base_aux_alloc_no_alloc<_Alloc>(_Al)//, comp(_Parg)
...
template<class _Kty,
	...
	class set8
		: public _Tree_set8<_Tset8_traits<_Kty, _Pr, _Alloc, false> >
	{	// ordered red-black tree of key values, unique keys
public:
	typedef set8<_Kty, _Pr, _Alloc> _Myt;
	typedef _Tree_set8<_Tset8_traits<_Kty, _Pr, _Alloc, false> > _Mybase;
	typedef _Kty key_type;
	typedef _Pr key_compare;
	...
	set8()
		...
		{	// construct empty set8 from defaults
		}

	explicit set8(const key_compare& _Pred)
		: _Mybase(_Pred, allocator_type())
		{	// construct empty set8 from comparator
		}

	set8(const key_compare& _Pred, const allocator_type& _Al)
		: _Mybase(_Pred, _Al)
		{	// construct empty set8 from comparator and allocator
		}

	template<class _Iter>
		set8(_Iter _First, _Iter _Last)
		: _Mybase(key_compare(), allocator_type())
		{	// construct set8 from [_First, _Last), defaults
		_DEBUG_RANGE(_First, _Last);
		for (; _First != _Last; ++_First)
			this->insert(*_First);
		}
	...
	template<class _Iter>
		...
		: _Mybase(_Pred, allocator_type())
		{	// construct set8 from [_First, _Last), comparator
		_DEBUG_RANGE(_First, _Last);
		for (; _First != _Last; ++_First)
			this->insert(*_First);
		}
	...
	template<class _Iter>
		...
		: _Mybase(_Pred, _Al)
		{	// construct set8 from [_First, _Last), defaults, and allocator
		_DEBUG_RANGE(_First, _Last);
		for (; _First != _Last; ++_First)
			this->insert(*_First);
		}
	...
	// set8 implements a performant swap
template<class _Kty, class _Pr, class _Alloc>
	class _Move_operation_category<set8<_Kty, _Pr, _Alloc> >
	{
	public:
...
template<class _Kty,
	class _Pr,
	class _Alloc> inline
	void swap(set8<_Kty, _Pr, _Alloc>& _Left,
		set8<_Kty, _Pr, _Alloc>& _Right)
