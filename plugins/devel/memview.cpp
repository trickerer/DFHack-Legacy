...
{
    //for (auto &r : ranges)
    for (std::vector12<t_memrange>::iterator i = ranges.begin(); i != ranges.end(); ++i)
        if ((*i).isInRange(trg))
            return true;
    return false;
}
...
DFhackCExport command_result plugin_onupdate (color_ostream &out)
{
    ...
    Core::getInstance().proc->read(memdata.addr,memdata.len,memdata.buf);
...
{
    mymutex->lock();
    Core::getInstance().proc->getMemRanges(memdata.ranges);
    if (parameters.empty())
    {
        memdata.addr = 0;
    }
