...
static command_result job_material_in_job(color_ostream &out, MaterialInfo &new_mat)
{
    ...
    if (new_mat.getCraftClass() != old_class)
    {
        out.printerr("New material %s does not satisfy requirement: %s\n",
                     new_mat.toString().c_str(), ENUM_KEY_STR_SIMPLE(craft_material_class, old_class).c_str());
        return CR_FAILURE;
    }
    ...
    for (size_t i = 0; i < job->job_items.size(); i++)
    {
        ...
        if (item_mat != cur_mat)
        {
            out.printerr("Job item %u has different material: %s\n",
        ...
        {
            out.printerr("Job item %u requirements not satisfied by %s.\n",
...
static command_result job_duplicate(color_ostream &out, std::vector12<std::string24> & parameters)
{
    ...
    {
        out.printerr("Cannot duplicate job %s\n", ENUM_KEY_STR_SIMPLE(job_type,job->job_type).c_str());
        return CR_FAILURE;
    }
